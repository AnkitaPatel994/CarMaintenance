<?php
	...
	if(isset($data_back->{"c_d_id"}) && isset($data_back->{"startdate"}) && isset($data_back->{"enddate"}))
	{
		if(!empty($data_back->{"c_d_id"}) && !empty($data_back->{"startdate"}) && !empty($data_back->{"enddate"}))
		{
			$c_d_id=$data_back->{"c_d_id"};
			$startdate=$data_back->{"startdate"};
			$enddate=$data_back->{"enddate"};
			
			$query = "select c_id,c_dshift,d_name,client_name,c_other,c_cash,c_gastype,c_gascash,c_maintenance,c_commission,c_gst,c_cashleft,c_total,c_date from driver,dailycashout,client where client_id = c_c_id and c_d_id = d_id and c_d_id = '".$c_d_id."' and c_date between '".$startdate."' and '".$enddate."'";
